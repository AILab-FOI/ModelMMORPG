...
class AnswerNPC(spade.Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        try:
            print "{}: Answering to NPC {} with {} ...".format(
                self.myAgent.name.split('@')[0],
                self.npcID,
                self.answer)
...
class stopTalkingToNPC(spade.Behaviour.OneShotBehaviour):
    """docstring for stopTalkingToNPC"""
    ...
    def _process(self):
        try:
            print "{}: Stopping communication with NPC {} ...".format(
                self.myAgent.name.split('@')[0],
                self.npcID)
...
class goToNPC(spade.Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        try:
            print "{}: Going to NPC {} at location {} {}-{} ...".format(
                self.myAgent.name.split('@')[0],
                self.npc,
                self.mapID,
                self.x,
...
class goToLocation(spade.Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        try:
            print "{}: Going to location {}-{}-{} ...".format(
                self.myAgent.name.split('@')[0],
                self.mapID,
                self.x,
                self.y)
...
class equipItem(spade.Behaviour.OneShotBehaviour):
    ...
    def _process(self):
        try:
            print "{}: Trying to equip item in slot {} ...".format(
                self.myAgent.name.split('@')[0],
                self.slot)
...
class killMob(spade.Behaviour.OneShotBehaviour):
    """docstring for talkToNPC"""
    ...
    def _process(self):
        try:
            print("{}: Trying to attack a {} ...".format(
                self.myAgent.name.split('@')[0],
                self.mobname))
            ...
            # return False
            self.myAgent.result = True  # the behaviour was run successfully, although no mobs were killed
        except Exception as e:
            print e
            # return False
            self.myAgent.result = False
...
class randomWalk(spade.Behaviour.OneShotBehaviour):
        ...
        try:
            ...
            if nearnpc:
                ...
                try:
                    ...
                    # return True
                    self.myAgent.result = True
                    self.kill()  # kill the behaviour (a la return from behaviour)
                except Exception as e:
                    print e
                    # return False
                    self.myAgent.result = False
...
class Reason(spade.Behaviour.Behaviour):
    def say(self, msg):
        ...
        print '%s: %s' % (self.myAgent.name.split('@')[0], str(msg))
    ...
    def updateObjectives(self):
        ...
        print "Kay."
        try:
            self.say('My avatar name is ' + self.myAgent.avatar_name)
        except Exception as e:
            print e
    ...
    def _process(self):
        ...
        self.say('Updating my knowledge base ...')
        self.updateKB()

        self.say('Updating my objectives ...')
        obj = self.updateObjectives()
        self.say('Selecting an objective ...')
        if obj and obj[0]['Name'] != 'random_walk':
            next = self.selectObjective(obj)
        else:
            next = 'random_walk'
        self.say('Planning quest "%s" ...' % next)
        start = self.planQuest(next)
        if not start:
            self.say('Cannot plan or start quest ...')
            return
        planDone = False
        while not planDone:
            nextAction = self.getNextAction(next)
            if not nextAction:
                self.say("My plan didn't work out. I'll try something else.")
                break
            self.say('My next action is "%s(%s)"' % (nextAction[0], ','.join(nextAction[1])))
            self.act(nextAction)
            counter = 0
            self.say('I made my move, let us see if this worked ...')
            ...
            while self.myAgent.result is False:
                ...
                counter += 1
                if counter > 10:
                    self.say("An ERROR must have occured when running the behaviour!")
                    break

            if not self.myAgent.result:
                self.say('Action failed miserably, need to rethink my options ...')
                break
            ...
            while not success or retries == 0:
                ...
                self.say("Updating my knowledge base ...")
                self.updateKB()
                self.say("Testing if action was successful ...")
                try:
                    success = self.actionDone(nextAction)
            ...
            if not success:
                self.say('The action was unsuccessful, need to rethink my options ...')
                self.actionFailed()
                break # action has been unsuccessful

            self.say('The action was successfull!')
            ...
            if planDone:
                self.say('I finished quest "%s"! Going for the next one ...' % next)
                break


            if self.reconsider():
                self.say('It seems to be time to reconsider my options ...')
                self.say('Updating my knowledge base ...')
                self.updateKB()
                self.say('Updating my objectives ...')
                self.say('woah!')
